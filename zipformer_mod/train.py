...
from asr_datamodule_generic import SpeechAsrDataModule
...
logger = logging.getLogger('icefall')
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if batch_idx % params.valid_interval == 0 and not params.print_diagnostics:
            ...
            model.train()
            logger.info(f"Epoch {params.cur_epoch}, validation: {valid_info}")
            logger.info(
...
def run(rank, world_size, args):
    ...
    logger.info(f"Number of model parameters: {num_param}")
    ...
    speechdata = SpeechAsrDataModule(args)

    train_cuts = speechdata.train_cuts()
    ...
    train_dl = speechdata.train_dataloaders(
        train_cuts, sampler_state_dict=sampler_state_dict
    )


    valid_cuts = speechdata.dev_cuts()
    valid_dl = speechdata.valid_dataloaders(valid_cuts)
    ...
    if checkpoints and "grad_scaler" in checkpoints:
        logger.info("Loading grad scaler state dict")
...
def main():
    parser = get_parser()
    SpeechAsrDataModule.add_arguments(parser)
